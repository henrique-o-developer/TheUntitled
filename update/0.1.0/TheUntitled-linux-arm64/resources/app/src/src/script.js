...
window.addEventListener("DOMContentLoaded", async function () {
    const fs = require('fs');
    const dir = "./src/mega_databases";
    const path = "./resources/app/src/mega_databases";

    if (fs.existsSync(`./src`)){
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
        }
    } else if (fs.existsSync(`./resources`)){
        if (!fs.existsSync(path)) {
            fs.mkdirSync(path);
        }
    }
    UsersDB = new Mega.crearDB("Users")
    now()
